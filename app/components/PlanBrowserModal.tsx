...
import { X, ChevronRight, ChevronLeft, Clock, BarChart3, Calendar, Info, Search, SlidersHorizontal } from "lucide-react";
  ...
  const filtered = useMemo(() => {
    return PLAN_LIBRARY.filter((p) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!p.name.toLowerCase().includes(q) && !p.split.toLowerCase().includes(q) && !p.sequence.toLowerCase().includes(q)) return false;
      }
      return true;
    });
      ...
      <div className="relative w-full max-w-lg max-h-[94vh] bg-[#080d18] border border-white/[0.08] rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden">
        ...
        {!detail ? (
          <>
            ...
            <div className="px-5 pt-5 pb-3 shrink-0">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-[10px] font-mono text-white/30 mt-0.5">{filtered.length} plans</p>
                </div>
            ...
            {/* Plan List */}
            <div className="flex-1 overflow-y-auto custom-scroll px-4 py-3 space-y-1.5">
              {filtered.length === 0 ? (
                ...
                filtered.map((plan) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left rounded-xl border border-white/[0.05] bg-white/[0.015] p-3.5 hover:border-[rgb(var(--accent-rgb)/0.2)] hover:bg-white/[0.03] transition group"
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <p className="text-[13px] font-semibold text-white/85 leading-tight">{plan.name}</p>
