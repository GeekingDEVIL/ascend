...
export function HoloCard({ icon, label, value, sub }: { icon: React.ReactNode; label: string; value: string; sub?: string }) {
  return (
    ...
    >
      <div className="w-12 shrink-0 flex items-center justify-center bg-[rgb(var(--accent-rgb)/0.1)] border-r border-[rgb(var(--accent-rgb)/0.2)] text-[rgb(var(--accent-light-rgb))]">
        {icon}
      </div>
      <div className="flex-1 p-3">
        <p className="text-xs font-bold text-white/85 tracking-wide mb-2">{label}</p>
        <div className="flex items-center justify-between rounded border border-[rgb(var(--accent-rgb)/0.2)] bg-[rgb(var(--accent-rgb)/0.4)] px-2.5 py-1.5">
          <span className="text-[10px] text-[rgb(var(--accent-light-rgb)/0.6)] font-mono tracking-widest">{sub ?? "VALUE"}</span>
...
export function rankForLevel(level: number) {
  if (level >= 50) return { name: "PLATINUM", color: "text-[rgb(var(--accent-light-rgb))]", border: "border-[rgb(var(--accent-light-rgb)/0.4)]", glow: "rgb(var(--accent-light-rgb) / 0.6)" };
