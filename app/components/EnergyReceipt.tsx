...
const SOURCE_LABELS: Record<ValueSource, { text: string; color: string }> = {
  ...
  default: { text: "DEFAULT", color: "text-[var(--fg-30)]" },
...
function ReceiptRow({ sv, unit }: { sv: SystemValue<number | string>; unit?: string }) {
  ...
  return (
    <div className="py-2 border-b border-dashed border-[var(--fg-06)] last:border-0">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-mono text-[var(--fg-50)]">{sv.label}</span>
        <div className="flex items-center gap-2">
          <span className={`text-[7px] font-mono px-1.5 py-0.5 rounded bg-[var(--fg-04)] ${src.color}`}>{src.text}</span>
          <span className="text-sm font-bold font-mono text-[var(--fg-80)]">
            {typeof sv.value === "number" ? sv.value : sv.value}
            {unit && <span className="text-[9px] text-[var(--fg-25)] ml-0.5">{unit}</span>}
          </span>
        </div>
      </div>
      {sv.confidence && (
        <p className="text-[8px] font-mono text-[var(--fg-15)] mt-0.5">±{Math.round((sv.confidence[1] - sv.confidence[0]) / 2)} kcal ({sv.confidence[0]}–{sv.confidence[1]})</p>
      )}
      {sv.detail && <p className="text-[8px] font-mono text-[var(--fg-20)] mt-0.5">{sv.detail}</p>}
      {sv.because.length > 0 && (
        <div className="mt-1 space-y-0.5">
          {sv.because.map((b, i) => (
            <p key={i} className="text-[7px] font-mono text-[var(--fg-15)]">
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="text-[10px] font-mono text-[var(--fg-30)] hover:text-[var(--fg-60)] transition">CLOSE</button>
        </div>
        ...
        <div className="mt-4 pt-3 border-t border-[var(--fg-06)]">
          <p className="text-[9px] font-mono text-[var(--fg-25)] mb-2">MACROS</p>
          <div className="space-y-0">
        ...
        {receipt.adjustments.length > 0 && (
          <div className="mt-4 pt-3 border-t border-[var(--fg-06)]">
            <p className="text-[9px] font-mono text-[var(--fg-25)] mb-2">ADJUSTMENTS</p>
        ...
        {receipt.guardrailViolations.length > 0 && (
          <div className="mt-4 pt-3 border-t border-amber-400/20">
            ...
            {receipt.guardrailViolations.map((v, i) => (
              <div key={i} className="py-1.5 border-b border-dashed border-[var(--fg-04)] last:border-0">
                <div className="flex items-start gap-2">
                  <span className="text-amber-300/50 text-[9px] mt-0.5 shrink-0">⚠</span>
                  <div>
                    <p className="text-[9px] font-mono text-[var(--fg-40)]">{v.rule}</p>
                    <p className="text-[8px] font-mono text-[var(--fg-20)]">{v.detail}</p>
        ...
        <p className="text-[7px] font-mono text-[var(--fg-10)] mt-4 text-center">Every number has a source. No magic.</p>
      </div>
    </div>
  );
}
