...
const LeaderboardCard = React.forwardRef<HTMLDivElement, LeaderboardCardProps>(
  ({ className, title = "Leaderboard", fromDate, toDate, podiumRankings, rankings, currentUserId, runOptions, selectedRunId, onRunChange, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("rounded-2xl border border-[var(--fg-06)] bg-[var(--fg-02)] p-5", className)}
        {...props}
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div className="space-y-0.5">
            <h3 className="text-lg font-bold text-[var(--fg-90)]">{title}</h3>
          ...
          {runOptions && runOptions.length > 0 && (
            <select
              ...
              className="text-[10px] font-mono bg-[var(--fg-04)] text-[var(--fg-60)] rounded-lg border border-[var(--fg-08)] px-2.5 py-1.5 outline-none focus:border-[rgb(var(--accent-rgb)/0.3)]"
            >
