  ...
  return (
    ...
    >
      {items.map((item) => (
        ...
        >
          {item.icon && (
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-[var(--fg-04)] border border-[var(--fg-06)]">
              {item.icon}
            </div>
          )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-[13px] font-medium text-[var(--fg-85)] truncate">
                {item.label}
              </p>
              {item.badge && (
                <span
            ...
            {item.subtitle && (
              <p className="text-[10px] font-mono text-[var(--fg-30)] mt-0.5 truncate">
                {item.subtitle}
              </p>
            )}
            {item.progress && (
              <div className="mt-2 h-1.5 rounded-full bg-[var(--fg-06)] overflow-hidden">
                <div
          ...
          <ChevronRight
            size={14}
            className="text-[var(--fg-15)] group-hover:text-[var(--fg-30)] transition shrink-0"
          />
        </motion.button>
      ))}
    </motion.div>
