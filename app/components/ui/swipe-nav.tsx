...
import { motion, LayoutGroup, PanInfo, useMotionValue, animate } from "framer-motion";
  ...
  const goTo = useCallback(
    (targetIdx: number): boolean => {
      if (targetIdx >= 0 && targetIdx < sections.length && targetIdx !== idx) {
        router.push(sections[targetIdx].key, { scroll: false });
        return true;
      }
      return false;
    },
          ...
          {sections.map((s, i) => {
            ...
            return (
              <button
                ...
                onClick={() => goTo(i)}
                className="relative px-2.5 py-1.5 rounded-md transition-[background-color] duration-200 shrink-0"
                style={{
              ...
              >
                <span
                  ...
                  style={{
                    color: isActive ? `rgb(${s.colorRgb})` : "var(--fg-40)",
                  }}
                >
                  {s.label}
                </span>
