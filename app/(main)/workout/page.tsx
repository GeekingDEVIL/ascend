...
function CardPanel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={`rounded-2xl border border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)] ${className}`}>{children}</div>
    );
}
...
function StatCell({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
    return (
        <div className="glass-card px-3 py-2.5 text-center">
            <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.25)] mb-0.5">{label}</p>
            <p className={`text-lg font-bold font-mono ${accent ? "text-[rgb(var(--accent-rgb))]" : "text-[rgb(var(--fg-rgb)/0.90)]"}`}>{value}</p>
        </div>
    );
}
...
export default function WorkoutPage() {
    ...
    async function generateShareImage(): Promise<Blob | null> {
        ...
        ctx.fillStyle = "rgb(var(--fg-rgb) / 0.3)";
        ...
        ctx.fillStyle = "rgb(var(--fg-rgb) / 0.4)";
        ...
        stats.forEach(([label, value], i) => {
            ...
            ctx.strokeStyle = "rgb(var(--fg-rgb) / 0.1)";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.roundRect(x, y, cellW, cellH, 20);
            ctx.stroke();
            ctx.fillStyle = "rgb(var(--fg-rgb) / 0.35)";
            ctx.font = "24px ui-monospace, monospace";
        ...
        ctx.fillStyle = "rgb(var(--fg-rgb) / 0.2)";
        ctx.font = "24px ui-monospace, monospace";
        ...
        <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-4 pb-24 relative">
            <div className="relative z-10 max-w-xl mx-auto pt-2">
                ...
                <div className="flex flex-col items-center justify-center py-16">
                    ...
                    <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mt-1.5 max-w-xs mx-auto text-center">Recovery is when your muscles grow. Nothing to log today.</p>
                </div>
            </div>
        </main>
    );
        ...
        <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-4 pb-24 relative">
            <div className="relative z-10 max-w-xl mx-auto pt-2">
                ...
                <div className="text-center mb-8 mt-8">
                    ...
                    <p className="text-lg font-bold text-[rgb(var(--fg-rgb)/0.85)]">Get Started</p>
                    <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mt-1 max-w-xs mx-auto">Choose how you want to train</p>
                </div>

                <div className="space-y-3">
                    <div className="rounded-2xl border border-[rgb(var(--accent-rgb)/0.15)] bg-[rgb(var(--accent-rgb)/0.03)] p-4">
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] mb-1">Create personalized plan</p>
                        <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mb-3">Build your own weekly schedule with custom exercises, sets, and rest days.</p>
                    ...
                    <div className="rounded-2xl border border-[rgb(var(--accent-rgb)/0.15)] bg-[rgb(var(--fg-rgb)/0.03)] p-4">
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] mb-1">Select from existing plans</p>
                        <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mb-3">Browse proven workout programs — PPL, Upper/Lower, Full Body, and more.</p>
                    ...
                    <div className="rounded-2xl border border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)] p-4">
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] mb-1">Just train today</p>
                        <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mb-3">No plan needed — pick exercises and log as you go.</p>
                        <button onClick={() => setStatus("freestyle")} className="w-full text-sm font-medium py-3 rounded-xl border border-[rgb(var(--fg-rgb)/0.1)] text-[rgb(var(--fg-rgb)/0.60)] hover:text-[rgb(var(--fg-rgb)/0.90)] hover:bg-[rgb(var(--fg-rgb)/0.05)] transition">
    ...
    if (status === "freestyle") return (
        <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pb-24 relative">
            <div className="relative z-10 max-w-xl mx-auto px-4 pt-6">
                <button onClick={() => { setFreestyleExercises([]); setStatus(exercisesList.length > 0 ? "not_started" : "no_plan"); }} className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.30)] hover:text-[rgb(var(--fg-rgb)/0.60)] transition mb-4">
                    ← Back
                </button>
                <h1 className="text-xl font-bold font-display text-[rgb(var(--accent-light-rgb))] mb-1">Freestyle Session</h1>
                <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.30)] mb-5">Pick exercises and start training</p>
                ...
                {freestyleExercises.length === 0 ? (
                    <div className="text-center py-10">
                        <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.25)]">No exercises added yet.</p>
                    </div>
                ) : (
                    <div className="space-y-2 mb-6">
                        {freestyleExercises.map((ex, i) => (
                            <div key={ex.id} className="flex items-center gap-3 glass-card px-4 py-3">
                                <span className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.20)] w-5 shrink-0">{String(i + 1).padStart(2, "0")}</span>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[13px] font-medium text-[rgb(var(--fg-rgb)/0.80)] truncate">{ex.name}</p>
                                    <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">{ex.body_segment}</p>
                                </div>
                                <button onClick={() => removeFreestyleExercise(ex.id)} className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-[rgb(var(--fg-rgb)/0.20)] hover:text-red-400 transition">
                                    <X size={14} />
                                </button>
                            </div>
                        ))}
            ...
            <div className="relative z-10 w-full max-w-xl mx-auto pt-2 space-y-3">
                ...
                <CardPanel className="p-5">
                    <div className="text-center mb-5">
                        ...
                        <p className="text-[9px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.25)] mb-1">COMPLETED TODAY</p>
                        <p className="text-lg font-bold text-[rgb(var(--fg-rgb)/0.90)]">{dayTitle}</p>
                    </div>
                        ...
                        <div className="space-y-2 mb-4">
                            {todaySessions.map((s, i) => (
                                <div key={s.id} className="glass-card p-3">
                                    <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.25)] mb-2">SESSION {i + 1}</p>
                                    <div className="grid grid-cols-4 gap-2 text-center">
                                        <div><p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">TIME</p><p className="text-sm font-bold font-mono text-[rgb(var(--fg-rgb)/0.80)]">{formatClock(s.duration)}</p></div>
                                        <div><p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">SETS</p><p className="text-sm font-bold font-mono text-[rgb(var(--fg-rgb)/0.80)]">{s.sets}</p></div>
                                        <div><p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">VOL</p><p className="text-sm font-bold font-mono text-[rgb(var(--fg-rgb)/0.80)]">{Math.round(kgToUnit(s.volume, weightUnit)).toLocaleString()}</p></div>
                            ...
                            <div className="rounded-xl border border-[rgb(var(--accent-rgb)/0.15)] bg-[rgb(var(--accent-rgb)/0.05)] p-3">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center">
                                    <div><p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">SESSIONS</p><p className="text-sm font-bold font-mono text-[rgb(var(--fg-rgb)/0.80)]">{todaySessions.length}</p></div>
                                    <div><p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">TOTAL SETS</p><p className="text-sm font-bold font-mono text-[rgb(var(--fg-rgb)/0.80)]">{todaySessions.reduce((a, s) => a + s.sets, 0)}</p></div>
                    ...
                    <p className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.20)] text-center mb-4">Nice work! You can start another session or come back tomorrow.</p>

                    <div className="flex gap-2">
                        <button onClick={() => router.push("/schedule")} className="flex-1 text-sm font-medium py-3 rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.50)] hover:text-[rgb(var(--fg-rgb)/0.80)] hover:bg-[rgb(var(--fg-rgb)/0.05)] transition">
                        ...
                        <button onClick={handleShare} disabled={sharing} title="Share" className="shrink-0 w-11 flex items-center justify-center rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.30)] hover:text-[rgb(var(--fg-rgb)/0.60)] disabled:opacity-40 transition">
                            {sharing ? <div className="w-4 h-4 border-2 border-[rgb(var(--fg-rgb)/0.20)] border-t-[rgb(var(--accent-rgb))] rounded-full animate-spin" /> : <Share2 size={14} />}
                        </button>
                    </div>
                    {todaySessions.length >= MAX_SESSIONS_PER_DAY ? (
                        <p className="w-full mt-2 text-[10px] font-mono py-2.5 text-center text-[rgb(var(--fg-rgb)/0.20)]">
                        ...
                        <button
                            ...
                            className="w-full mt-2 text-[10px] font-mono py-2.5 rounded-xl border border-[rgb(var(--fg-rgb)/0.06)] text-[rgb(var(--fg-rgb)/0.30)] hover:text-[rgb(var(--fg-rgb)/0.60)] hover:border-[rgb(var(--fg-rgb)/0.15)] transition"
                        >
                            ...
                            {recentSessions.map((s) => {
                                ...
                                return (
                                    <div key={s.id} className="flex items-center justify-between py-2 px-1">
                                        <span className="text-sm text-[rgb(var(--fg-rgb)/0.70)] truncate flex-1 min-w-0">{s.title}</span>
                                        <span className="text-xs font-mono text-[rgb(var(--fg-rgb)/0.25)] shrink-0 ml-2">{label}</span>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <CardPanel className="p-4">
                        ...
                        <div className="flex items-center gap-4 mb-3">
                            <div className="relative w-16 h-16 shrink-0">
                                ...
                                <span className="absolute inset-0 flex items-center justify-center text-xs font-mono font-bold text-[rgb(var(--fg-rgb)/0.80)]">{doneCount}/7</span>
                            </div>
                            <div>
                                <p className="text-sm font-medium text-[rgb(var(--fg-rgb)/0.70)]">{doneCount === 0 ? "No sessions yet" : `${doneCount} session${doneCount !== 1 ? "s" : ""} done`}</p>
                                <p className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.25)] mt-0.5">{7 - doneCount} day{7 - doneCount !== 1 ? "s" : ""} remaining</p>
                            </div>
                        </div>
                            ...
                            {DAY_LABELS.map((day, i) => {
                                ...
                                return (
                                    <div key={day} className={`flex-1 py-1.5 rounded-lg text-center text-[10px] font-mono font-semibold transition-colors ${
                                            ...
                                            : "bg-[rgb(var(--fg-rgb)/0.04)] text-[rgb(var(--fg-rgb)/0.20)]"
                                    }`}>{day}</div>
                                );
                            })}
                        </div>
                ...
                {statsLoaded && weeklyVolumes.some(v => v > 0) && (
                    <CardPanel className="p-4">
                                    ...
                                    return (
                                        <div key={i} className="flex-1 flex flex-col items-center gap-1">
                                            <div className="w-full relative flex-1 flex items-end">
                                                <div className={`w-full rounded-t-md transition-all ${isLatest ? "bg-[rgb(var(--accent-rgb))]" : "bg-[rgb(var(--fg-rgb)/0.08)]"}`} style={{ height: `${h}%` }} />
                                            </div>
                                            <span className="text-[7px] font-mono text-[rgb(var(--fg-rgb)/0.20)]">{isLatest ? "NOW" : `W${i + 1}`}</span>
                                        </div>
                                    );
                                });
                            })()}
                        ...
                        <div className="flex justify-between mt-2">
                            <span className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.20)]">6 weeks</span>
                            <span className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.30)]">{weeklyVolumes[weeklyVolumes.length - 1] > 0 ? `${(weeklyVolumes[weeklyVolumes.length - 1] / 1000).toFixed(1)}k ${weightUnit}` : "—"}</span>
                        </div>
                    </CardPanel>
                )}
            </div>
            ...
            {showFreestylePrompt && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="w-full max-w-sm rounded-2xl border border-[rgb(var(--fg-rgb)/0.08)] bg-[var(--bg-card)] p-5">
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] mb-2">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowFreestylePrompt(false)} className="flex-1 text-sm font-medium py-2.5 rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.50)] hover:text-[rgb(var(--fg-rgb)/0.80)] transition">
                                No Thanks
                            </button>
        ...
        <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pb-36 md:pb-10 relative">
            ...
            <div className="relative z-10 max-w-xl mx-auto px-4 pt-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-[9px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] uppercase">
                            ...
                            <h1 className="text-lg font-bold text-[rgb(var(--fg-rgb)/0.90)] leading-tight">{dayTitle}</h1>
                        </div>
                        {status === "active" && (
                            <div className="text-right shrink-0 rounded-xl border border-[rgb(var(--accent-rgb)/0.15)] bg-[rgb(var(--accent-rgb)/0.05)] px-3 py-1.5">
                                <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)]">ELAPSED</p>
                                <p className="text-lg font-bold font-mono text-[rgb(var(--accent-rgb))]">{formatClock(elapsed)}</p>
                            </div>
                        )}
                        {status === "not_started" && (
                            <div className="flex items-center gap-2 shrink-0">
                                <button onClick={() => setShowDeletePlanConfirm(true)} className="w-9 h-9 flex items-center justify-center rounded-xl border border-[rgb(var(--fg-rgb)/0.06)] text-[rgb(var(--fg-rgb)/0.20)] hover:text-red-400/80 hover:border-red-500/20 transition">
                                    <Trash2 size={13} />
                                </button>
                                <button onClick={() => router.push("/schedule")} className="flex items-center gap-1.5 text-[10px] font-mono px-3 py-2 rounded-xl border border-[rgb(var(--fg-rgb)/0.06)] text-[rgb(var(--fg-rgb)/0.30)] hover:text-[rgb(var(--fg-rgb)/0.60)] hover:bg-[rgb(var(--fg-rgb)/0.03)] transition">
                ...
                {status === "active" && (
                    <div className="flex items-center gap-3">
                        <div className="flex-1 h-1.5 rounded-full bg-[rgb(var(--fg-rgb)/0.06)] overflow-hidden">
                        ...
                        <p className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.25)] shrink-0">{completedCount}/{totalPlanned}</p>
                    </div>
                )}
                    ...
                    <div className={`rounded-2xl border p-4 ${
                        ...
                        <div className="flex items-start justify-between gap-3 mb-2">
                            ...
                            <div className="text-right shrink-0">
                                <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.20)]">INTENSITY</p>
                        ...
                        <p className="text-[10px] text-[rgb(var(--fg-rgb)/0.35)] leading-relaxed">{cycleProfile.banner.detail}</p>

                        {/* Energy Forecast */}
                        {status === "not_started" && energyForecast.length > 0 && (
                            <div className="mt-3 pt-3 border-t border-[rgb(var(--fg-rgb)/0.04)]">
                                <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] mb-2">7-DAY ENERGY FORECAST</p>
                                    ...
                                    {energyForecast.map((f, i) => {
                                        ...
                                        return (
                                            <div key={f.date} className="flex-1 flex flex-col items-center gap-1">
                                                <span className="text-[7px] font-mono text-[rgb(var(--fg-rgb)/0.20)]">{f.label.slice(0, 3)}</span>
                                                ...
                                                <span className={`text-[8px] font-mono ${i === 0 ? "text-[rgb(var(--fg-rgb)/0.50)] font-bold" : "text-[rgb(var(--fg-rgb)/0.20)]"}`}>{dayLabel}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                        ...
                        {status === "not_started" && (
                            <div className="mt-3 pt-3 border-t border-[rgb(var(--fg-rgb)/0.04)] grid grid-cols-2 gap-2">
                                <div className="glass-card p-2.5">
                                    <p className="text-[7px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] mb-1">WARM-UP · {cycleProfile.warmUpGuidance.minutes} MIN</p>
                                    <p className="text-[9px] text-[rgb(var(--fg-rgb)/0.35)] leading-relaxed">{cycleProfile.warmUpGuidance.focus}</p>
                                </div>
                                <div className="glass-card p-2.5">
                                    <p className="text-[7px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] mb-1">NUTRITION TIP</p>
                                    <p className="text-[9px] text-[rgb(var(--fg-rgb)/0.35)] leading-relaxed">{cycleProfile.nutritionTip}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="relative overflow-hidden rounded-2xl border border-[rgb(var(--accent-rgb)/0.15)] bg-gradient-to-br from-[rgb(var(--accent-rgb)/0.08)] to-transparent">
                        ...
                        <div className="relative p-5">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="flex items-center gap-2 text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.30)]">
                                    <Dumbbell size={14} className="text-[rgb(var(--accent-rgb)/0.5)]" />
                                    <span>{exercisesList.length} exercises</span>
                                    <span className="text-[rgb(var(--fg-rgb)/0.10)]">·</span>
                            ...
                            <div className="flex items-center justify-between mt-3">
                                <button onClick={() => setStatus("freestyle")} className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.20)] hover:text-[rgb(var(--fg-rgb)/0.50)] transition">
                ...
                {status === "not_started" && (
                    <div className="flex items-center gap-3 rounded-xl border border-[rgb(var(--fg-rgb)/0.05)] bg-[rgb(var(--fg-rgb)/0.02)] px-4 py-2.5">
                        <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.25)] shrink-0">BODY WEIGHT</p>
                        <input
                            ...
                            placeholder="—"
                            className="flex-1 min-w-0 h-8 rounded-lg bg-[rgb(var(--fg-rgb)/0.04)] border border-[rgb(var(--fg-rgb)/0.06)] text-center text-sm font-bold font-mono focus:outline-none focus:border-[rgb(var(--accent-rgb)/0.3)] transition"
                        />
                        <span className="text-[10px] font-mono text-[rgb(var(--fg-rgb)/0.20)] shrink-0">{weightUnit}</span>
                ...
                {status === "not_started" && statsLoaded && (recentSessions.length > 0 || weeklyVolumes.some(v => v > 0)) && (
                    <div className="space-y-3 mt-2">
                        <p className="text-[9px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.15)] px-1">YOUR TRAINING</p>

                        {/* This Week + Volume side-by-side */}
                        <div className="grid grid-cols-2 gap-3">
                            <div className="rounded-2xl border border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)] p-3.5">
                                <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] mb-2">THIS WEEK</p>
                                <p className="text-3xl font-black text-[rgb(var(--accent-light-rgb))] leading-none">{weekDays.filter(Boolean).length}</p>
                                <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.20)] mt-1">of 7 days</p>
                                <div className="flex gap-1 mt-3">
                                    {["M", "T", "W", "T", "F", "S", "S"].map((day, i) => (
                                        <div key={i} className={`flex-1 h-1.5 rounded-full transition-all ${
                                            weekDays[i] ? "bg-[rgb(var(--accent-rgb))]" : "bg-[rgb(var(--fg-rgb)/0.06)]"
                                        }`} />
                                    ))}
                                </div>
                            </div>
                            <div className="rounded-2xl border border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)] p-3.5">
                                <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)] mb-2">VOLUME</p>
                                <p className="text-3xl font-black text-[rgb(var(--fg-rgb)/0.80)] leading-none">
                                    {weeklyVolumes[weeklyVolumes.length - 1] > 0 ? `${(weeklyVolumes[weeklyVolumes.length - 1] / 1000).toFixed(1)}` : "—"}
                                </p>
                                <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.20)] mt-1">{weeklyVolumes[weeklyVolumes.length - 1] > 0 ? `k ${weightUnit} this week` : "no data yet"}</p>
                                    ...
                                    <div className="flex items-end gap-0.5 mt-3 h-4">
                                        {(() => {
                                            ...
                                            return weeklyVolumes.map((vol, i) => {
                                                ...
                                                const isLatest = i === weeklyVolumes.length - 1;
                                                return <div key={i} className={`flex-1 rounded-sm transition-all ${isLatest ? "bg-[rgb(var(--accent-rgb))]" : "bg-[rgb(var(--fg-rgb)/0.1)]"}`} style={{ height: `${h}%` }} />;
                                            });
                                        })()}
                                    </div>
                                )}
                        ...
                        {recentSessions.length > 0 && (
                            <div className="rounded-2xl border border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)] p-4">
                                <div className="flex items-center justify-between mb-3">
                                    <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.20)]">RECENT</p>
                                ...
                                <div className="space-y-1">
                                    {recentSessions.slice(0, 3).map((s) => {
                                        ...
                                        return (
                                            <div key={s.id} className="flex items-center gap-3 py-2 border-b border-[rgb(var(--fg-rgb)/0.03)] last:border-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[11px] font-medium text-[rgb(var(--fg-rgb)/0.60)] truncate">{s.title}</p>
                                                    <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.20)]">{label} · {s.sets} sets</p>
                                                </div>
                ...
                {status === "active" && (
                    <div className="space-y-3">
                        {exercisesList.map((ex, i) => {
                            ...
                            return (
                                <div key={ex.id} className={`rounded-xl border overflow-hidden transition-all ${isSkipped ? "border-[rgb(var(--fg-rgb)/0.04)] bg-[rgb(var(--fg-rgb)/0.01)] opacity-50" : allDone ? "border-[rgb(var(--accent-rgb)/0.2)] bg-[rgb(var(--accent-rgb)/0.03)]" : "border-[rgb(var(--fg-rgb)/0.06)] bg-[rgb(var(--fg-rgb)/0.03)]"}`}>
                                    {/* Exercise header */}
                                    <button onClick={() => isSkipped ? unskipExercise(ex.id) : setExpandedId(isOpen ? null : ex.id)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-[10px] font-mono font-bold ${isSkipped ? "bg-[rgb(var(--fg-rgb)/0.03)] text-[rgb(var(--fg-rgb)/0.15)] border border-[rgb(var(--fg-rgb)/0.04)]" : allDone ? "bg-[rgb(var(--accent-rgb)/0.15)] text-[rgb(var(--accent-rgb))] border border-[rgb(var(--accent-rgb)/0.2)]" : "bg-[rgb(var(--fg-rgb)/0.04)] text-[rgb(var(--fg-rgb)/0.20)] border border-[rgb(var(--fg-rgb)/0.06)]"}`}>
                                            {isSkipped ? <Ban size={12} /> : allDone ? <Check size={14} /> : String(i + 1).padStart(2, "0")}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-1.5">
                                                <p className={`text-[13px] font-medium truncate ${isSkipped ? "text-[rgb(var(--fg-rgb)/0.30)] line-through" : "text-[rgb(var(--fg-rgb)/0.80)]"}`}>{ex.name}</p>
                                        ...
                                        {isSkipped ? (
                                            <span className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.20)] shrink-0">tap to undo</span>
                                        ) : (
                                            <ChevronDown size={14} className={`text-[rgb(var(--fg-rgb)/0.15)] shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
                                        )}
                                    </button>
                                    ...
                                    {isOpen && !isSkipped && (
                                        <div className="border-t border-[rgb(var(--fg-rgb)/0.04)]">
                                                    ...
                                                    <div className="space-y-1.5">
                                                        {substitutions[ex.id].map((sub) => (
                                                            <button
                                                                ...
                                                                className="w-full flex items-center justify-between gap-2 rounded-md bg-[rgb(var(--fg-rgb)/0.03)] border border-[rgb(var(--fg-rgb)/0.06)] px-3 py-2 hover:border-emerald-400/20 hover:bg-emerald-400/[0.04] transition group"
                                                            >
                                                                <div className="min-w-0">
                                                                    <p className="text-xs text-[rgb(var(--fg-rgb)/0.80)] font-medium truncate">{sub.exercise.name}</p>
                                                                    <p className="text-[9px] font-mono text-[rgb(var(--fg-rgb)/0.30)]">{sub.reason} · {sub.exercise.equipment}</p>
                                                                </div>
                                            ...
                                            <div className="px-4 pt-2.5 pb-1 flex items-center gap-3">
                                                <button onClick={() => setSwapTargetId(ex.id)} className="flex items-center gap-1.5 text-[rgb(var(--fg-rgb)/0.40)] text-[10px] font-mono hover:text-emerald-400 active:scale-95 transition px-2 py-1.5 rounded-md hover:bg-emerald-400/10">
                                                    <RefreshCw size={11} /> Swap
                                                </button>
                                                <button onClick={() => skipExercise(ex.id)} className="flex items-center gap-1.5 text-[rgb(var(--fg-rgb)/0.40)] text-[10px] font-mono hover:text-amber-400 active:scale-95 transition px-2 py-1.5 rounded-md hover:bg-amber-400/10">
                                                    <SkipForward size={11} /> Skip
                                                </button>
                                                {done === 0 && (
                                                    <button onClick={() => removeExercise(ex.id)} className="flex items-center gap-1.5 text-[rgb(var(--fg-rgb)/0.40)] text-[10px] font-mono hover:text-red-400 active:scale-95 transition px-2 py-1.5 rounded-md hover:bg-red-400/10">
                                                        <X size={11} /> Remove
                                                    </button>
                                                )}
                                                {!ex.isCardio && !ex.isBodyweight && (
                                                    <button onClick={() => toggleWarmup(ex)} className={`flex items-center gap-1.5 text-[10px] font-mono transition ml-auto px-2 py-1.5 rounded-md active:scale-95 ${warmupExercises.has(ex.id) ? "text-amber-400 bg-amber-400/10 hover:bg-amber-400/15" : "text-[rgb(var(--fg-rgb)/0.40)] hover:text-amber-400 hover:bg-amber-400/10"}`}>
                                                        <Flame size={11} />
                                            ...
                                            {ex.isCardio ? (
                                                ...
                                                <div className="px-4 pb-4 pt-2 space-y-3">
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <div>
                                                            <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.30)] mb-1">DURATION (MIN)</p>
                                                            <input
                                                                ...
                                                                placeholder="—"
                                                                className="w-full h-12 rounded-lg bg-[rgb(var(--fg-rgb)/0.04)] border border-[rgb(var(--fg-rgb)/0.08)] text-center text-xl font-bold font-mono focus:outline-none focus:border-[rgb(var(--accent-rgb)/0.4)] disabled:opacity-40 transition"
                                                            />
                                                        </div>
                                                        <div>
                                                            <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.30)] mb-1">DISTANCE (KM)</p>
                                                            <input
                                                        ...
                                                        <div>
                                                            <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.30)] mb-1">SPEED (KM/H)</p>
                                                            <input
                                                                type="number" min="0" inputMode="decimal"
                                                                onWheel={(e) => (e.target as HTMLElement).blur()}
                                                                value={sets[0]?.weight ?? ""}
                                                        ...
                                                        <div>
                                                            <p className="text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.30)] mb-1">INCLINE (%)</p>
                                                            <input
                                                                type="number" min="0" inputMode="decimal"
                                                                onWheel={(e) => (e.target as HTMLElement).blur()}
                                                                value={sets[0]?.reps ?? ""}
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full text-[10px] font-mono rounded-lg bg-transparent border border-[rgb(var(--fg-rgb)/0.06)] px-3 py-2 text-[rgb(var(--fg-rgb)/0.40)] placeholder:text-[rgb(var(--fg-rgb)/0.15)] focus:outline-none focus:border-[rgb(var(--accent-rgb)/0.2)] disabled:opacity-40 transition"
                                                    />
                                                    {!sets[0]?.completed ? (
                                                        <button
                                                ...
                                                <div className="px-4 pb-4 space-y-1.5 border-t border-[rgb(var(--fg-rgb)/0.06)] pt-3">
                                                    <div className="flex items-center gap-1.5 sm:gap-2 text-[8px] font-mono text-[rgb(var(--fg-rgb)/0.25)] tracking-wider">
                                                        ...
                                                        <div key={s.index} className={isWarmup ? "rounded-lg bg-amber-400/[0.04] border border-amber-400/[0.08] px-1 py-0.5" : ""}>
                                                            <SwipeSet completed={s.completed} onComplete={() => completeSet(ex, s.index)}>
                                                                <div className={`flex items-center gap-1.5 sm:gap-2 ${isWarmup ? "bg-transparent" : "bg-[var(--bg-elevated)]"}`}>
                                                                    <span className={`text-[10px] font-mono w-5 sm:w-7 text-center shrink-0 ${isWarmup ? "text-amber-400/50" : s.completed ? "text-[rgb(var(--accent-light-rgb)/0.5)]" : "text-[rgb(var(--fg-rgb)/0.25)]"}`}>
                                                                        {displayNum}
                                                                    </span>
                                                                    ...
                                                                    {ex.isBodyweight ? (
                                                                        <input type="number" min="0" inputMode="numeric" onWheel={(e) => (e.target as HTMLElement).blur()} placeholder="—" value={s.reps} onChange={(e) => updateSet(ex.id, s.index, "reps", e.target.value)} disabled={s.completed}
                                                                            className={`flex-1 min-w-0 h-10 sm:h-11 rounded-lg border text-center text-sm sm:text-base font-bold font-mono focus:outline-none disabled:opacity-40 transition ${isWarmup ? "bg-amber-400/[0.03] border-amber-400/[0.1] focus:border-amber-400/30" : "bg-[rgb(var(--fg-rgb)/0.04)] border-[rgb(var(--fg-rgb)/0.08)] focus:border-[rgb(var(--accent-rgb)/0.4)] focus:bg-[rgb(var(--accent-rgb))]/[0.03]"}`} />
                                                                    ) : (
                                                                        <>
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className={`w-9 h-9 sm:w-11 sm:h-11 shrink-0 rounded-lg border flex items-center justify-center transition ${s.completed
                                                                            ...
                                                                            : isWarmup ? "border-amber-400/15 text-amber-400/30 hover:border-amber-400/40 hover:text-amber-400/70 active:scale-95" : "border-[rgb(var(--fg-rgb)/0.10)] text-[rgb(var(--fg-rgb)/0.20)] hover:border-[rgb(var(--accent-rgb)/0.4)] hover:text-[rgb(var(--accent-light-rgb))] hover:bg-[rgb(var(--accent-rgb))]/[0.05] active:scale-95"
                                                                            }`}
                                                                    >
                                                                        <Check size={16} />
                                                                    </button>
                                                            ...
                                                            {!s.completed && !isWarmup && (
                                                                <input type="text" value={s.note} onChange={(e) => updateSet(ex.id, s.index, "note", e.target.value)} placeholder="Note (optional)"
                                                                    className="mt-1 ml-5 sm:ml-7 text-[10px] font-mono rounded-md bg-transparent border border-[rgb(var(--fg-rgb)/0.04)] px-2 py-1 text-[rgb(var(--fg-rgb)/0.30)] placeholder:text-[rgb(var(--fg-rgb)/0.15)] focus:outline-none focus:border-[rgb(var(--accent-rgb)/0.2)] focus:text-[rgb(var(--fg-rgb)/0.50)] transition" style={{ width: "calc(100% - 24px)" }} />
                                                            )}
                                                        </div>
                                                        );
                                                    })}
                        ...
                        <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 text-[rgb(var(--fg-rgb)/0.20)] text-xs font-mono hover:text-[rgb(var(--fg-rgb)/0.50)] transition py-2">
            ...
            {restRemaining !== null && status === "active" && (
                <div className="fixed bottom-16 md:bottom-6 left-0 right-0 md:left-1/2 md:-translate-x-1/2 md:max-w-sm md:rounded-xl z-30">
                    <div className="border-t md:border border-[rgb(var(--fg-rgb)/0.08)] bg-[var(--bg-card)]/95 backdrop-blur-xl px-5 py-3.5 flex items-center justify-between md:rounded-xl">
                        <div>
                            <p className="text-[8px] font-mono tracking-widest text-[rgb(var(--fg-rgb)/0.25)]">REST TIMER</p>
                            <p className="text-2xl font-bold font-mono text-[rgb(var(--accent-rgb))]">{formatClock(restRemaining)}</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <button onClick={() => setRestRemaining((r) => (r !== null ? r + 15 : null))} className="text-[10px] font-mono px-2.5 py-2 rounded-lg border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.40)] hover:text-[rgb(var(--fg-rgb)/0.70)] active:scale-95 transition">+15s</button>
                            <button onClick={() => setRestPaused((p) => !p)} className="w-10 h-10 flex items-center justify-center rounded-lg border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.40)] hover:text-[rgb(var(--fg-rgb)/0.70)] active:scale-95 transition">
                                {restPaused ? <Play size={16} /> : <Pause size={16} />}
                            </button>
                            <button onClick={() => { setRestRemaining(null); setRestPaused(false); }} className="w-10 h-10 flex items-center justify-center rounded-lg border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.40)] hover:text-[rgb(var(--fg-rgb)/0.70)] active:scale-95 transition">
                                <SkipForward size={16} />
                            </button>
                        </div>
                    </div>
            ...
            {status === "active" && restRemaining === null && (
                <div className="fixed bottom-16 md:bottom-6 left-0 right-0 md:left-1/2 md:-translate-x-1/2 md:max-w-sm md:rounded-xl z-20">
                    <div className="border-t md:border border-[rgb(var(--fg-rgb)/0.06)] bg-[var(--bg-card)]/95 backdrop-blur-xl px-5 py-3 md:rounded-xl flex items-center gap-2">
                        {!sessionPaused ? (
                            <>
                                <button
                                    onClick={() => {
                                    ...
                                    className="text-[10px] font-mono font-medium py-3 px-3 rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.40)] hover:text-[rgb(var(--fg-rgb)/0.70)] transition"
                                >
            ...
            {showEndConfirm && !finishing && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="w-full max-w-sm rounded-2xl border border-[rgb(var(--fg-rgb)/0.08)] bg-[var(--bg-card)] p-5">
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] mb-2">End workout?</p>
                        {completedCount === 0 ? (
                            <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.35)] mb-4">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowEndConfirm(false)} className="flex-1 text-sm font-medium py-2.5 rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.50)] hover:text-[rgb(var(--fg-rgb)/0.80)] transition">
                                Keep Going
                            </button>
            ...
            {showDeletePlanConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="w-full max-w-sm rounded-2xl border border-red-500/15 bg-[var(--bg-card)] p-5">
                        ...
                        <p className="text-sm font-semibold text-[rgb(var(--fg-rgb)/0.85)] text-center mb-2">Delete entire workout plan?</p>
                        <p className="text-[11px] text-[rgb(var(--fg-rgb)/0.35)] text-center mb-4">
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setShowDeletePlanConfirm(false)} className="flex-1 text-sm font-medium py-2.5 rounded-xl border border-[rgb(var(--fg-rgb)/0.08)] text-[rgb(var(--fg-rgb)/0.50)] hover:text-[rgb(var(--fg-rgb)/0.80)] transition">
                                Cancel
                            </button>
                            <button onClick={deletePlan} disabled={deletingPlan} className="flex-1 text-sm font-semibold py-2.5 rounded-xl bg-red-500 text-[var(--text-primary)] hover:bg-red-600 disabled:opacity-50 transition">
