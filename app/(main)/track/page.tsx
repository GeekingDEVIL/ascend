      ...
      {data.map((val, i) => {
        ...
        return (
          <div
            ...
            style={{
              height: `${h}%`,
              backgroundColor: isLast ? color : "var(--fg-08)",
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full -rotate-90">
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--fg-04)" strokeWidth="4" />
        <circle
      ...
      <span className="absolute inset-0 flex items-center justify-center text-sm font-mono font-bold text-[var(--fg-80)]">
        {pct}%
      </span>
    </div>
  );
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g.key} className="flex items-center gap-2">
            <span className="text-[9px] font-mono text-[var(--fg-30)] w-16 text-right shrink-0">{mg?.label ?? g.key}</span>
            <div className="flex-1 h-2 rounded-full bg-[var(--fg-04)] overflow-hidden">
              <div
                className="h-full rounded-full transition-all duration-500"
                style={{ width: `${Math.max(pct, 3)}%`, backgroundColor: mg?.color ?? "rgb(var(--accent-rgb))" }}
              />
            </div>
            <span className="text-[9px] font-mono text-[var(--fg-20)] w-7 shrink-0">{pct}%</span>
          </div>
        );
      })}
    </div>
      ...
      >
        ...
        <motion.div variants={staggerItem} className="glass-card p-4" onClick={() => router.push("/progress")} style={{ cursor: "pointer" }}>
          ...
          <div className="flex items-baseline gap-2 mt-2">
            ...
            <span className="text-xs font-mono text-[var(--fg-25)]">{weightUnit} this week</span>
          </div>
        </motion.div>
        ...
        <motion.div variants={staggerItem} className="grid grid-cols-3 gap-2">
          <div className="glass-card p-3 text-center">
            ...
            <p className="text-[9px] font-mono text-[var(--fg-20)] mt-0.5">records</p>
          </div>
          <div className="glass-card p-3 text-center">
            ...
            <p className={`text-[9px] font-mono mt-0.5 ${
              stats.bodyWeightChange !== null
                ? stats.bodyWeightChange > 0 ? "text-orange-300/50" : stats.bodyWeightChange < 0 ? "text-emerald-300/50" : "text-[var(--fg-20)]"
                : "text-[var(--fg-20)]"
            }`}>
          ...
          <div className="glass-card p-3 text-center">
            ...
            <p className="text-[9px] font-mono text-[var(--fg-20)] mt-0.5">sessions</p>
          </div>
        </motion.div>
        ...
        {loaded && (
          <motion.div variants={staggerItem} className="glass-card p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-mono text-[var(--fg-25)]">{stats.monthSessions}/{stats.monthTarget} target</span>
            </div>
            <div className="h-2 rounded-full bg-[var(--fg-04)] overflow-hidden">
              <motion.div
        ...
        <motion.div variants={staggerItem} className="grid grid-cols-5 gap-2.5">
          ...
          <div className="col-span-2 glass-card p-4 flex flex-col items-center justify-center">
            ...
            {loaded && stats.recoveryPct !== null ? (
              ...
              <div className="w-16 h-16 rounded-full border-4 border-[var(--fg-04)] flex items-center justify-center">
                <span className="text-sm font-mono text-[var(--fg-20)]">—</span>
              </div>
            )}
          ...
          <div className="col-span-3 glass-card p-4">
            ...
            {loaded && stats.muscleGroups.length > 0 ? (
              ...
              <p className="text-[10px] font-mono text-[var(--fg-15)] text-center py-6">
