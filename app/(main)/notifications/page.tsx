...
const DEFAULT_TYPE_META = { icon: (s: number) => <Bell size={s} className="text-[var(--fg-40)]" />, bg: "bg-[var(--fg-04)] border-[var(--fg-08)]", label: "Other", chipActive: "border-[var(--fg-30)] bg-[var(--fg-10)] text-[var(--fg-70)]" };
      ...
      <div className="relative z-10 max-w-xl mx-auto px-4 pt-6 space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-[11px] text-[var(--fg-30)] mt-0.5">
              {unreadCount > 0 ? `${unreadCount} unread` : "All caught up"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            {unreadCount > 0 && (
              <button onClick={dismissAll} className="text-[10px] font-mono px-3 py-1.5 rounded-xl border border-[var(--fg-06)] text-[var(--fg-30)] hover:text-[var(--fg-60)] hover:bg-[var(--fg-03)] transition">
                Mark All Read
              </button>
            )}
        ...
        <div className="space-y-2">
          <div className="flex gap-2">
            <button onClick={() => setFilter("all")} className={`text-[10px] font-mono px-3 py-1.5 rounded-lg border transition ${filter === "all" ? "border-[rgb(var(--accent-rgb)/0.2)] bg-[rgb(var(--accent-rgb)/0.08)] text-[rgb(var(--accent-rgb))]" : "border-[var(--fg-06)] text-[var(--fg-30)] hover:text-[var(--fg-60)]"}`}>
              ALL
            </button>
            <button onClick={() => setFilter("unread")} className={`text-[10px] font-mono px-3 py-1.5 rounded-lg border transition ${filter === "unread" ? "border-[rgb(var(--accent-rgb)/0.2)] bg-[rgb(var(--accent-rgb)/0.08)] text-[rgb(var(--accent-rgb))]" : "border-[var(--fg-06)] text-[var(--fg-30)] hover:text-[var(--fg-60)]"}`}>
              UNREAD {unreadCount > 0 && `(${unreadCount})`}
            </button>
            {typeFilter !== "all" && (
              <button onClick={() => setTypeFilter("all")} className="text-[10px] font-mono px-3 py-1.5 rounded-lg border border-[var(--fg-06)] text-[var(--fg-25)] hover:text-[var(--fg-60)] transition ml-auto">
                Clear type ✕
              </button>
            )}
          </div>
              ...
              {presentTypes.map((t) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 text-[9px] font-mono px-2.5 py-1 rounded-full border transition ${active ? m.chipActive : "border-[var(--fg-06)] text-[var(--fg-30)] hover:text-[var(--fg-55)] hover:border-[var(--fg-15)]"}`}
                  >
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <div className="w-14 h-14 mx-auto mb-4 rounded-2xl glass-card flex items-center justify-center">
              <Bell size={24} className="text-[var(--fg-15)]" />
            </div>
            <p className="text-sm font-semibold text-[var(--fg-30)]">All Caught Up</p>
            <p className="text-xs text-[var(--fg-20)] mt-1">
          ...
          <div className="space-y-5">
            {grouped.map((group) => (
              <div key={group.label}>
                <p className="text-[9px] font-mono tracking-widest text-[var(--fg-20)] mb-2">{group.label.toUpperCase()}</p>
                <motion.div className="space-y-1.5" variants={staggerContainer} initial="hidden" animate="visible">
                  {group.items.map((n) => (
                    <motion.div
                      variants={staggerItem}
                      key={n.id}
                      className={`flex items-center gap-3 rounded-xl border border-[var(--fg-06)] px-3.5 py-3 transition ${n.read ? "bg-[var(--fg-01)] opacity-50" : "bg-[var(--fg-03)]"}`}
                    >
                      <NotifIcon type={n.type} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-[12px] font-semibold ${n.read ? "text-[var(--fg-50)]" : "text-[var(--fg-90)]"}`}>{n.title}</p>
                        {n.message && <p className="text-[11px] text-[var(--fg-35)] mt-0.5 truncate">{n.message}</p>}
                        <p className="text-[9px] font-mono text-[var(--fg-20)] mt-1">{timeAgo(n.created_at)}</p>
                      </div>
                      {!n.read && (
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="w-2 h-2 rounded-full bg-[rgb(var(--accent-rgb))]" />
                          <button onClick={() => dismiss(n.id)} className="text-[var(--fg-20)] hover:text-[var(--fg-50)] transition">
                            <X size={14} />
                          </button>
                        </div>
                      )}
